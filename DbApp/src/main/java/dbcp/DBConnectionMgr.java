...
public class DBConnectionMgr {
    private Vector connections = new Vector(10); //DB 연결 객체 갯수 미리 10개 준비
    ...
    public static DBConnectionMgr getInstance() { // 객체생성-> DBCP의 주소 가져옴
        if (instance == null) {
    ...
    public synchronized Connection getConnection() // 한번에 1개처리, 
            throws Exception {
        if (!initialized) {
    ...
    public synchronized void freeConnection(Connection c) { // 반납
